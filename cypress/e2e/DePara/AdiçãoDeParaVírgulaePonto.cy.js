describe('Login válido', () => {
  it('Deverá realizar o Login', () => {
    ...
    cy.get('#fromToName').should('be.visible').type('TST Cypress 002');
    cy.get('#description').should('be.visible').type('TST Cypress 002');
    cy.get('.border-2').should('be.visible').click();
    cy.get('#inputFrom').should('be.visible').type('0,1');
    cy.get('#inputTo').should('be.visible').type('0.2');
